    ...
    <?php echo form_open($this->uri->uri_string(), 'id="frm"'); ?>
        <fieldset>
                ...
                <div class='controls'>
                	<select name="TINGKAT_PENDIDIKAN_ID" id="TINGKAT_PENDIDIKAN_ID" class="form-control">
						...
						<?php foreach($tk_pendididikan_all as $record):?>
                            <?php 
                                $selected = "";
                                if($record->ID ==$selectedData->TINGKAT_PENDIDIKAN_ID){
                                    $selected = " SELECTED ";
                                }
                            ?>
							<option <?php echo $selected; ?> value="<?php echo $record->ID?>"> <?php echo $record->NAMA; ?></option>
						<?php endforeach;?>
					</select>
            ...
            <div class="control-group<?php echo form_error('NAMA') ? ' error' : ''; ?> col-sm-12">
                ...
                <div class='controls'>
                    <input id='NAMA' type='text' class="form-control" name='NAMA' maxlength='32' value="<?php echo set_value('NAMA', isset($selectedData->NAMA) ? $selectedData->NAMA : ''); ?>" />
