...
<div class='box box-warning' id="form-riwayat-jabatan-add">
    ...
    <?php echo form_open($this->uri->uri_string(), 'id="frmjabatanmandiri"'); ?>
    <fieldset>
				...
				<div class='controls'>
                    <select id="ID_UNOR" name="ID_UNOR" width="100%" class="select2 col-md-10 format-control">
						...
						<?php foreach($recunor as $record):?>
							<option value="<?php echo $record->ID?>" <?php if(isset($detail_riwayat->ID_UNOR))  echo  (trim($detail_riwayat->ID_UNOR)==trim($record->KODE_INTERNAL)) ? "selected" : ""; ?> <?php if(isset($detail_riwayat->ID_JENIS_JABATAN))  echo  ($jenis_jabatan == $record->ID) ? "selected" : ""; ?>><?php echo $record->NAMA_UNOR; ?></option>
							<?php endforeach;?>
						<?php endif;?>
					</select>
                </div>
  		...
  		<div class="box-footer">
  			<a href="javascript:;" id="btnsavejabatanmandiri" class="btn green btn-primary button-submit"> Kirimkan Data
...
<script>
	$("#btnsavejabatanmandiri").click(function(){
		$('#btnsavejabatanmandiri').text('Sedang Mengirim...');
        $('#btnsavejabatanmandiri').addClass('disabled');
		submitdata_jabatan_mandiri();
		return false; 
	});	
	...
	function submitdata_jabatan_mandiri(){
		var the_data = new FormData(document.getElementById("frmjabatanmandiri"));
		var json_url = "<?php echo base_url() ?>pegawai/riwayatjabatan/savemandiri";
		 $.ajax({    
		 	type: "POST",
			url: json_url,
			data: the_data,
			...
			success: function(data){ 
                if(data.success){
                    ...
                    $("#modal-custom-global").trigger("sukses-tambah-riwayat-jabatan");
					 ...
					 $('#btnsavejabatanmandiri').removeClass('disabled');
                     $('#btnsavejabatanmandiri').text('Kirimkan Data');
                }
                else {
                	$('#btnsavejabatanmandiri').removeClass('disabled');
                    $('#btnsavejabatanmandiri').text('Kirimkan Data');
