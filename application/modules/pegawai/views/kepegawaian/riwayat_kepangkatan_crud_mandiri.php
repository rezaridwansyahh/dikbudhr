...
<div class="callout callout-info">
   <h4>Perhatian</h4>
   <p>Data yang anda masukan akan diverifikasi oleh kepegawaian Satker/Upt anda setelah itu data akan diverifikasi ulang oleh Biro SDM</p>
 </div>
<div class='box box-warning' id="form-kepangkatan-add-mandiri">
	<div class="box-body">
        <div class="messages">
        </div>
    <?php echo form_open($this->uri->uri_string(), 'id="frmkepangkatanmandiri"'); ?>
        <fieldset>
            ...
            <div class="control-group col-sm-3">
                ...
                <div class='controls'>
                    ...
                    <?php if(isset($detail_riwayat->FILE_BASE64) && $detail_riwayat->FILE_BASE64 != ""){ ?>
                        <a href="<?php echo base_url(); ?>pegawai/riwayatkepangkatan/viewdoc/<?php echo $detail_riwayat->DIKLAT_FUNGSIONAL_ID; ?>" class="btn btn-warning show-modal" tooltip="Lihat dokumen <?php echo isset($detail_riwayat->SK_NOMOR) ? $detail_riwayat->SK_NOMOR : ''; ?>" ><span class="fa fa-file-o"></span> Lihat</a>
                    <?php } ?>  
                </div>
            </div>
        </div>
  		<div class="box-footer">
        <a href="javascript:;" id="btnsave_kepangkatan_mandiri" class="btn green btn-primary button-submit"> Simpan Data
...
<script>
    var form = $("#form-kepangkatan-add-mandiri");
...
<script>
	$("#btnsave_kepangkatan_mandiri").click(function(){
        $('#btnsave_kepangkatan_mandiri').text('Sedang Mengirim...');
        $('#btnsave_kepangkatan_mandiri').addClass('disabled');
		submitdata_kepangkatan_mandiri();
		return false; 
	});	
	...
	function submitdata_kepangkatan_mandiri(){
		var the_data = new FormData(document.getElementById("frmkepangkatanmandiri"));
		var json_url = "<?php echo base_url() ?>pegawai/riwayatkepangkatan/savemandiri";
		 $.ajax({    
		 	type: "POST",
			url: json_url,
			data: the_data,
			...
			success: function(data){ 
                if(data.success){
                    ...
                    $("#modal-custom-global").trigger("sukses-tambah-riwayat-kepangkatan");
                     ...
                     $('#btnsave_kepangkatan_mandiri').removeClass('disabled');
                     $('#btnsave_kepangkatan_mandiri').text('Simpan Data');
                }
                else {
                    $('#btnsave_kepangkatan_mandiri').removeClass('disabled');
                    $('#btnsave_kepangkatan_mandiri').text('Simpan Data');
                    $("#form-kepangkatan-add-mandiri .messages").empty().append(data.msg);
                }
			}});
		return false; 
	}
