...
class Layanan_kpo_model extends BF_Model
{
	...
	public function find_all()
    {
		...
		if(empty($this->selects))
		{
			$this->select('p.*,
			(
				select "NAMA_JENIS_HUKUMAN" from rwt_hukdis  where p."NIP_BARU" = "PNS_NIP" and "ID_JENIS_HUKUMAN" = \'07\' LIMIT 1
			) as rwt_hukdis_kpo,
			(
				select "NILAI_PPK" from rwt_prestasi_kerja  where p."NIP_BARU" = "PNS_NIP" order by "TAHUN" desc limit 1
			) as last_ppk,
			'.$this->table_name.'.id as usulan_id,'.$this->table_name.'.no_surat_pengantar as no_surat_pengantar,no_surat_pengantar_es1,l.name as layanan_name,vw_satker."NAMA_UNOR" AS nama_satker,vw."NAMA_UNOR_FULL",'.$this->table_name.".status".','.$this->table_name.".id",false);
		}
		$this->db->where('p.NIP_BARU <>',null);
