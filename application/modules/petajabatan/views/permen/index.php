...
$num_columns	= 2;
$can_delete	= $this->auth->has_permission('Peraturan_otk.Masters.Delete');
$can_edit		= $this->auth->has_permission('Peraturan_otk.Masters.Edit');
	...
	<div class="box-header">
		<h3>
			 <?php if ($this->auth->has_permission('Petajabatan.Masters.Create')) : ?>
              <a href="<?php echo site_url($areaUrl . '/create_permen'); ?>">
...
var grid_daftar = $(".table-datatable").DataTable({
	...
	serverSide: true,
	ajax: {
	  url: "<?php echo base_url() ?>admin/masters/petajabatan/getdata_permen",
	  type:'POST',
	}
});

$('body').on('click','.btn-hapus-permen',function () { 
	...
	function (isConfirm) {
		if (isConfirm) {
			...
			$.ajax({
					url: "<?php echo base_url() ?>admin/masters/petajabatan/delete_permen",
					type:"POST",
					data: post_data,
					dataType: "html",
					timeout:180000,
