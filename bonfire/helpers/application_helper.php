...
if (! function_exists('log_activity_pegawai')) {
	/**
	 * Log an activity if config item 'enable_activities' is true.
	 *
     * @param integer $userId   The id of the user that performed the activity.
     ...
     * if enable_activity_logging is not true).
	 */
	function log_activity_pegawai($nip = "",$userId = null, $activity = '', $module = 'any')
	{
		$ci =& get_instance();
		
			$ci->load->model('pegawai/log_transaksi_model');
           return $ci->log_transaksi_model->log_activity_pegawai($nip,$userId, $activity, $module);
