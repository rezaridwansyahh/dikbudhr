...
?>
    ...
    <fieldset>
        ...
        <div class="control-group col-sm-12">
            <label for="satkers" class="control-label">Pilih Satker</label>
            <div class="controls">
                <?php
                    $asatkers = json_decode($user->satkers);
                    //print_r($asatkers);
                    ?>
                <select name="satkers[]" id="satkers" multiple="multiple" class="select2 <?php echo $controlClass; ?> form-control">
                    <option $selected value=''>Pilih Satker</option>
                    <?php

                    //bool array_key_exists( $key, $array )

                    if (isset($satkers) && is_array($satkers) && count($satkers)) :
                        foreach ($satkers as $satker) :
                            $selected = '';
                            $bool =  in_array( $satker->ID, $asatkers );
                            if($bool){
                                $selected = "selected";
                            }
                            echo "<option $selected value='$satker->ID'>".strtoupper($satker->NAMA_UNOR)."</option>";
                        endforeach;
                    endif;
                    ?>
                </select>
                <span class='help-inline'>Masukan Satker jika akses dibatasi pada satker yang telah ditentukan</span>
            </div>
        </div>
    </fieldset>
	<!--
