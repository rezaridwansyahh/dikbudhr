...
class Auth
{
	...
	public function loginapi($login, $password, $remember = false)
    {
        $status_user = "";
        if (empty($login) || empty($password)) {
            $status_user = "silahkan isi username dan password";
            return false;
        }
        ...
        if ($user == false) {
            $status_user = "User tidak ditemukan";
            return false;
        }
        ...
        if ($user->deleted >= 1) {
            ...
            $status_user = "Akun telah dihapus";
            return false;
        }
        ...
        if (! $this->check_password($password, $user->password_hash)) {
            // Bad password
            $status_user = "Password salah";
            return false;
        }

        // Check whether the account has been banned.
        if ($user->banned) {
             $status_user = "Akun dibanned";
        }
        $this->clear_login_attempts($login);
	...
	public function loginex($login, $password, $remember=FALSE)
	{
		if (empty($login) || empty($password))
		{
			$error = $this->ci->settings_lib->item('auth.login_type') == 'both' ? lang('bf_username') .'/'. lang('bf_email') : ucfirst($this->ci->settings_lib->item('auth.login_type'));
		...
		{
			die($password." | ".$user->password_hash);
			Template::set_message(lang('us_bad_email_pass'), 'error');
			$this->increase_login_attempts($login);
		}
