...
class User_model extends BF_Model
{
	...
	public function find($id=null)
	{
		...
		{
			$this->select($this->table_name . '.*, role_name,(select string_agg(role_id::text,\'#\'::text) as roles from roles_users where user_id = '.$this->table_name.'.id group by user_id ) as roles ');
		}

		$this->db->join($this->roles_table, $this->roles_table . '.role_id = ' . $this->table_name . '.role_id', 'left');

		$pegawai_data = parent::find($id);
		$roles = explode("#",$pegawai_data->roles);
		$pegawai_data->roles = $roles;
		return $pegawai_data;
	...
	public function find_all()
	{
		if (empty($this->selects))
		{
			$this->select($this->table_name . '.*, role_name,
				(select string_agg(role_id::text,\'#\'::text) as roles  from roles_users where user_id = '.$this->table_name.'.id group by user_id ) as roles 
			');
		}

		$this->db->join($this->roles_table, $this->roles_table . '.role_id = ' . $this->table_name . '.role_id', 'left');
		$pegawais_data =  parent::find_all();
		foreach($pegawais_data as $pegawai_data){
			$roles = explode("#",$pegawai_data->roles);
			$pegawai_data->roles = $roles;
		}
		return $pegawais_data;
	...
	public function find_by($field=null, $value=null, $type='and')
	{
		...
		$this->db->join($this->roles_table, $this->roles_table . '.role_id = ' . $this->table_name . '.role_id', 'left');
		$this->db->join("pegawai", 'pegawai.NIP_BARU = ' . $this->table_name . '.username', 'left');
	...
	public function admin_activation($user_id = FALSE)
	{
		...
		$result = $query->row();
		$this->update($result->id, array('activate_hash' => '','active' => 1,'banned' => 0));
	...
	public function getRoles($user_id){
		return $this->db->from("roles_users")->where('user_id',$user_id)->get()->result();
	}
}//end User_model